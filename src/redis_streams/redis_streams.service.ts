...
import { StreamHandlerService } from 'src/redis/stream-handler.service';
...
export class RedisStreamsService implements OnModuleInit, OnModuleDestroy {
	...
	public async consumeMessageFromGroup(
		...
		for await (const messageObj of generator) {
			...
			console.log('=>', {
				...
				consumer,
				counter,
				message: this.parseMessage(messageObj.message),
			});
			if (counter >= count) {
				break;
			}
	 ...
	 */
	public parseMessage(message: Record<string, string>) {
